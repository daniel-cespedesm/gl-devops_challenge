...
resource "aws_route_table_association" "jenkins_public1_assoc" {
...
resource "aws_instance" "jenkins_ec2" {
  ...
  provisioner "local-exec" {
    command = "aws ec2 wait-status-ok --instance-ids ${aws_instance.jenkins_ec2.id} --profile superhero && ansible-playbook -i ${aws_instance.jenkins_ec2.public_ip}, jenkins.yml"
  }
}
